...
DERIVATIVE state {       
   cai' = -ica * 2/(diam*FARADAY) * (1e4)  : koef could be 4 for univalent cations
}       
          
COMMENT       
This model uses ica but does not WRITE it; 
thus this model does not add anything to the total ionic current.       
...
cai0_ca_ion. Then this model's INITIAL block is called. 
By default, cai/cao would be set to the initial state values      
